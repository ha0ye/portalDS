...
Following [@Christensen_2018], we compute site-wide rodent abundances on the 8 control plots of the Portal Project from September 1979 through March 2015. Although data collection at this site is ongoing, treatments were changed on half of the plots in 2015; consequently, extending the time series to present would reduce the number of plots containing continuous observations of the community on control plots. More importantly, using a different set of plots would complicate comparisons with @Christensen_2018.

Because EDM methods rely on extracting signal from the data directly, it is important that individual time series contain enough variation for meaningful analysis. Thus, we pre-filter the set of species to those where at least half of the time points yielded one observation (i.e. non-zero observed abundance). This reduces the number of species from 21 to 7 (see Figure \@ref(fig:portal-time-series)).

Rodent abundances are aggregated across plots and scaled to the equivalent of 8 sampled plots; for some time points, not all 8 of the control plots were sampled, e.g. due to weather. The rodent censuses are typically scheduled to coincide with the new moon. However, this does not always occur, and so there may be missing censuses that interrupte the otherwise near-regular sampling in time. To address these missing censuses, we apply linear interpolation to construct time series with regular time intervals (1 time step = 1 lunar cycle) using the `na.interp()` function from the `forecast` R package [@Hyndman_2019].
